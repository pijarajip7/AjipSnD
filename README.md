...
| Aggressive entry (`InpAggressiveEntry`) | Off by default. When on, skips the whole rejection pattern — the FIRST wick into a saved zone triggers the market order immediately, no body/close-back-out requirement. SL anchors to the zone's own structural edge (`breakLevel`, sweep-aware) instead of the triggering bar's own wick, since that bar can close anywhere, including deep inside the zone. Unvalidated, not measured against the default |
