...
> rejection required — as a market order with structural SL/TP (stop
> anchored to the trigger bar's own wick, or to the zone's own edge in
> aggressive mode; RR-derived target). Risk-based lot sizing. Exit via
> broker SL/TP or daily/final close-all.
...
| Aggressive entry (`InpAggressiveEntry`) | Off by default. When on, skips the whole rejection pattern — the FIRST wick into a saved zone triggers the market order immediately, no body/close-back-out requirement, checked on every TICK (not just LTF bar close) so entry doesn't wait for the current bar to finish. SL anchors to the zone's own structural edge (`breakLevel`, sweep-aware) instead of the triggering bar's own wick, since the tick that touches the zone gives no bar body/close to anchor to. Unvalidated, not measured against the default |
...
| Structural SL/TP | SL anchored to the entry-trigger bar's own extreme + ATR buffer (rejection mode), or to the zone's own structural edge + ATR buffer (aggressive mode — see above); TP a multiple of the actual SL distance |
